...
Задание: измените программу, чтобы она загадывала число от 1 до 1_000_000 (1
...
#вариант 2
# Формально, в руби можно использовать case без выражения,
# но такая практика не считается очень удачной. Лучше так не делать.
=begin
number = rand(1..1_000_000)
print 'Привет! Я загадал число от 1 до 1 000 000, попробуйте угадать: '
input = nil
until input == number do #или while input != number
  input = gets.to_i
  case
  when input == number  then  puts 'Правильно!'
  when input >  number  then  print 'Искомое число меньше вашего ответа: '
  when input <  number  then  print 'Искомое число больше вашего ответа: '
  end
end
=end
